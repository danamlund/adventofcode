...
#+BEGIN_SRC cpp :results output
  #include <stdio.h>
  int main() {
    int numberToPrevTurn[10000] = {0};
    int numberToPrevPrevTurn[10000] = {0};

    int inputs[] = {1,0,15,2,10,13};
    int inputsi = 6;
    // int inputs[] = {0,3,6,-1};
    // int inputsi = 3;
    ...
    for (int turn = 1; turn <= 2020; turn++) {
      int newnumber = -1;
      if (turn <= inputsi) {
        newnumber = inputs[turn-1];
      } else if (!numberToPrevTurn[number]) {
        newnumber = 0;
      } else {
        newnumber = turn - 1 - numberToPrevTurn[number];// - numberToPrevPrevTurn[number];
        // printf("### %d = %d - %d\n", newnumber, turn - 1, numberToPrevTurn[number]);
      }
      //printf("## %3d: num=%d\n", turn, newnumber);
      //numberToPrevPrevTurn[number] = numberToPrevTurn[number];
      numberToPrevTurn[number] = turn-1;
      number = newnumber;
    }
...
: = 211
 ...
 - 211 correct <2020-12-17 Thu 18:23>
** part 2
#+BEGIN_SRC cpp :results output
  #include <stdio.h>
  #include <stdlib.h>
  int main() {
    //int numberToPrevTurn[1000000] = {0};
    int *numberToPrevTurn = (int*)malloc(30000001 * sizeof(int));
...
: = 2159626
 ...
 - 2159626 correct <2020-12-17 Thu 18:25>
* day 16
** puzzle
...
Your puzzle answer was 239727793813.

Both parts of this puzzle are complete! They provide two gold stars: **
** part 1
 - <2020-12-17 Thu 18:26>
#+BEGIN_SRC cpp :results output
  #include <stdio.h>
  #include <stdlib.h>
  #include <string.h>
  int main() {
    ...
    // fieldToMin1AndMax1AndMin2AndMax2
    int fs[22 * 4] = {0};
    ...
    while (fgets(line, sizeof(line), f)) {
      if (line[0] == '\n') break;
      char *ptr = index(line, ':') + 2;
      sscanf(ptr, "%d-%d or %d-%d", &fs[fsi+0], &fs[fsi+1], &fs[fsi+2], &fs[fsi+3]);
      fsi += 4;
    }
    for (int i = 0; i < fsi; i+=4) {
      printf("%d-%d or %d-%d\n", fs[i+0], fs[i+1], fs[i+2], fs[i+3]);
    }
    ...
    while (fgets(line, sizeof(line), f)) {
      ...
      do {
        ...
        int valid = 0;
        for (int i = 0; i < fsi; i+=4) {
          if (val >= fs[i+0] && val <= fs[i+1] || val >= fs[i+2] && val <= fs[i+3])
            valid = 1;
        }
        if (!valid) sum += val;
        //printf("%d, ", atoi(token));
...
#+end_example
 ...
 - 23115 correct <2020-12-17 Thu 18:46>
** part 2
#+BEGIN_SRC cpp :results output
  #include <stdio.h>
  #include <stdlib.h>
  ...
  int main() {
   ...
   char line[500];
    while (fgets(line, sizeof(line), f)) {
      if (line[0] == '\n') break;
      char *ptr = index(line, ':') + 2;
      if (0 == strncmp("departure", line, strlen("departure")))
        departues[fsi/4] = 1;
      sscanf(ptr, "%d-%d or %d-%d", &fs[fsi+0], &fs[fsi+1], &fs[fsi+2], &fs[fsi+3]);
      fsi += 4;
    }
    ...
    fgets(line, sizeof(line), f);
    fgets(line, sizeof(line), f);
    char *myticket = strdup(line);
    fgets(line, sizeof(line), f);
    fgets(line, sizeof(line), f);

    int fieldFits[22][22];
    for (int f = 0; f < fsi/4; f++) {
      for (int f2 = 0; f2 < fsi/4; f2++) {
        fieldFits[f][f2] = 1;
      }
    }
    ...
    while (fgets(line, sizeof(line), f)) {
      ...
      char *token = strtok(line, ",");
      do {
        vals[vali++] = atoi(token);
      } while (token = strtok(NULL, ","));

      int hasinvalid = 0;
      for (int f = 0; f < vali; f++) {
        int valid = 0;
        int val = vals[f];
        for (int i = 0; i < fsi; i+=4) {
          if (val >= fs[i+0] && val <= fs[i+1] || val >= fs[i+2] && val <= fs[i+3]) {
        ...
        if (!valid) { hasinvalid = 1; break; }
      }
      //printf("##2 %d\n", hasinvalid);
      if (!hasinvalid) {
        for (int f = 0; f < vali; f++) {
          int val = vals[f];
          for (int ff = 0; ff < vali; ff++) {
          // printf("## f=%d, ff=%d, val=%d,  %d-%d %d-%d, valid=%d, line=%s\n", 
          //   f, ff, val, fs[ff*4+0], fs[ff*4+1], fs[ff*4+2], fs[ff*4+3], 
          //   ((val >= fs[ff*4+0] && val <= fs[ff*4+1]) || (val >= fs[ff*4+2] && val <= fs[ff*4+3])), line);
            if (!(val >= fs[ff*4+0] && val <= fs[ff*4+1] || val >= fs[ff*4+2] && val <= fs[ff*4+3])) {
              fieldFits[f][ff] = 0;
            }
          }
        }
    ...
    while (true) {
      ...
      for (int f = 0; f < fsi/4; f++) {
        ...
        int fitsval = 0;
        for (int f2 = 0; f2 < fsi/4; f2++) {
          if (fieldFits[f][f2]) { fitsnum++; fitsval = f2; }
        }
        if (fitsnum == 1) {
          for (int f3 = 0; f3 < fsi/4; f3++) {
            if (f != f3 && fieldFits[f3][fitsval]) {
              fieldFits[f3][fitsval] = 0;
              changed = 1;
            }
          }
        }
    ...
    // for (int f = 0; f < fsi/4; f++) {
    //   printf("field %d fits: ", f);
    //   for (int f2 = 0; f2 < fsi/4; f2++) {
    //     if (fieldFits[f][f2]) printf("%d, ", f2);
    //   }
    //   printf("\n");
    // }

    {
      char *token = strtok(myticket, ",");
      int vali = 0;
      long sum = 1;
      do {
        int fitsval = 0;
        for (int f2 = 0; f2 < fsi/4; f2++) {
          if (fieldFits[vali][f2]) fitsval = f2;
        }
        if (departues[fitsval]) {
          //printf("## %d=%d\n", vali, atoi(token));
          sum *= atoi(token);
        }
        vali++;
...
: = 239727793813
 ...
 - 239727793813 correct <2020-12-17 Thu 19:26>
* day 17
** puzzle
--- Day 17: Conway Cubes ---
...
#+BEGIN_SRC cpp :results output
  ...
  char isset(struct xyzs **xyz, int x, int y, int z) {
    int hash = 7 + x;
    hash = hash * 31 + y;
    hash = hash * 31 + z;
    hash = hash % MAX;
    while (xyz[hash] != NULL) {
      if (xyz[hash]->x == x && xyz[hash]->y == y && xyz[hash]->z == z)
        return 1;
      hash = (hash + 1) % MAX;
    }
    return 0;
  }
  void set(struct xyzs **xyz, int x, int y, int z) {
    if (isset(xyz, x, y, z)) return;
    int hash = 7 + x;
    hash = hash * 31 + y;
    hash = hash * 31 + z;
    hash = hash % MAX;
    int limit = 10;
    while (xyz[hash] != NULL && limit++ >= 0)
      hash = (hash + 1) % MAX;
    if (limit == 0) { printf("limit!\n");exit(0);}

    xyz[hash] = (struct xyzs*) malloc(sizeof(struct xyzs));
    xyz[hash]->x = x;
    xyz[hash]->y = y;
    xyz[hash]->z = z;
  }
  int main() {
    ...
    struct xyzs **xyz = (struct xyzs**) malloc(sizeof(struct xyzs*)*MAX);
    for (int i = 0; i < MAX; i++) {
      xyz[i] = NULL;
    }
    ...
    struct xyzs **xyz2 = (struct xyzs**) malloc(sizeof(struct xyzs*)*MAX);

      // for (int z = minz; z <= maxz; z++) {
      //   printf("\nz=%d\n", z);
      //   for (int y = miny; y <= maxy; y++) {
      //     for (int x = minx; x <= maxx; x++) {
      //       printf("%c", isset(xyz, x, y, z) ? '#' : '.');
      //     }
      //     printf("\n");
      //   }
      // }
      ...
      for (int x = minx-1; x <= maxx+1; x++)
        for (int y = miny-1; y <= maxy+1; y++)
          for (int z = minz-1; z <= maxz+1; z++) {
            ...
            for (int dx = -1; dx <= 1; dx++)
              for (int dy = -1; dy <= 1; dy++)
                for (int dz = -1; dz <= 1; dz++) {
                  ...
                  if (isset(xyz2, x+dx, y+dy, z+dz)) neighbors++;
                }
            char eisset = isset(xyz2, x, y, z);
            if ((eisset && (neighbors == 2 || neighbors == 3))
                || (!eisset && neighbors == 3)) {
              set(xyz, x, y, z);
              if (x < minx) minx = x;
              if (x > maxx) maxx = x;
            ...
            // if (eisset || isset(xyz, x, y, z))
            //   printf("## %d,%d,%d  neighbors=%d, wasset=%d,  isset=%d\n", x, y, z, neighbors, eisset, isset(xyz, x, y, z));
          }
...
: = 215
 ...
 - 215 correct <2020-12-20 Sun 20:22>
** part 2
#+BEGIN_SRC cpp :results output
  #include <stdio.h>
  #include <stdlib.h>
  ...
  char isset(struct xyzs **xyz, int x, int y, int z, int w) {
    int hash = 7 + x;
    hash = hash * 31 + y;
    hash = hash * 31 + z;
    hash = hash * 31 + w;
    hash = hash % MAX;
    while (xyz[hash] != NULL) {
      if (xyz[hash]->x == x && xyz[hash]->y == y && xyz[hash]->z == z && xyz[hash]->w == w)
        return 1;
      hash = (hash + 1) % MAX;
    }
    return 0;
  }
  void set(struct xyzs **xyz, int x, int y, int z, int w) {
    if (isset(xyz, x, y, z, w)) return;
    int hash = 7 + x;
    hash = hash * 31 + y;
    hash = hash * 31 + z;
    hash = hash * 31 + w;
            ...
            for (int dx = -1; dx <= 1; dx++)
              for (int dy = -1; dy <= 1; dy++)
                ...
                for (int dw = -1; dw <= 1; dw++) {
                  ...
                  if (isset(xyz2, x+dx, y+dy, z+dz, w+dw)) neighbors++;
                }
            char eisset = isset(xyz2, x, y, z, w);
            if ((eisset && (neighbors == 2 || neighbors == 3))
                || (!eisset && neighbors == 3)) {
              set(xyz, x, y, z, w);
              if (x < minx) minx = x;
...
: = 1728
 ...
 - 1728 correct <2020-12-20 Sun 20:26>
...
#+BEGIN_SRC cpp :results output
  ...
  long calc(char *line) {
      ...
      char prevop = '?';
      while (true) {
          // printf("##1 out=%d prevop=%c line='%s'\n", out, prevop, line);
          long num = -1;
          if (line[0] == '(') {
              char paren[1024];
              int pareni = 0;
              int level = 1;
              for (int i = 1; level > 0; i++) {
                  if (line[i] == '(') level++;
                  if (line[i] == ')') level--;
                  paren[pareni++] = line[i];
              }
          ...
          line++;
          if (line[0] == '\n') { printf("4\n");exit(0); }
          if (out == -1) { out = num; prevop = op; continue; }
...
: = 1890866893020
 ...
 - 6796284051 too low <2020-12-21 Mon 13:14>
 - 1890866893020 correct <2020-12-21 Mon 13:32>
...
: = 34646237037193
 ...
 - 19590080143881 too low <2020-12-21 Mon 14:23>
 - 72087053649715973 too high <2020-12-21 Mon 14:33>
 - 34646237037193 correct <2020-12-21 Mon>
...
char* matchesrule(int rulei, char *msg, char *rulesstate);

char* matchesrules(char *rulesstring, char *msg, char *rulesstate) {
  char rulesstate2[200];
  for (int i = 0; i < ruleslen; i++) {
    rulesstate2[i] = rulesstate[i];
  }
  char *ptr = rulesstring - 1;
  while (ptr != NULL) {
    ptr++;
    int r = atoi(ptr);
    msg = matchesrule(r, msg, rulesstate);
    if (msg == NULL) {
      for (int i = 0; i < ruleslen; i++) {
        rulesstate[i] = rulesstate2[i];
      }
      return NULL;
    }
    ptr = index(ptr, ' ');
...
char* matchesrule(int rulei, char *msg, char *rulesstate) {
  char *rule = rules[rulei];
  //printf("## %d(%s)(%d) msg='%s'\n", rulei, rule, rulesstate[rulei], msg);
  if (rule[0] == '"') {
    return msg[0] == rule[1] ? msg+1 : NULL;
  }
  if (index(rule, '|') != NULL) {
    char tmp[100];
    strcpy(tmp, rule);
    char *rules1 = tmp;
    char *rules2 = index(tmp, '|');
    rules2[-1] = 0;
    rules2 += 2;
    int state = rulesstate[rulei];
    if (state == 0) {
      {
        char *out = matchesrules(rules1, msg, rulesstate);
        if (out != NULL) {
          state = 2;
          return out;
        }
      ...
      {
        char *out = matchesrules(rules2, msg, rulesstate);
        if (out != NULL) {
          state = 1;
          return out;
        }
  ...
  return matchesrules(rule, msg, rulesstate);
}
...
int main() {
  ...
  while (fgets(line, sizeof(line), f)) {
    ...
    if (line[0] == 0) break;
    int n = atoi(line);
    rules[n] = strdup(index(line, ':')+2);
    if (n > ruleslen) ruleslen = n;
  }
  ruleslen++;
  ...
  while (fgets(line, sizeof(line), f)) {
    ...
    for (int i = 0; i < ruleslen; i++) 
      rulesstate[i] = 0;
    char *out = matchesrule(0, line, rulesstate);
...
: = 107
 ...
 - 107 correct <2020-12-22 Tue 18:01>
** part 2
#+BEGIN_SRC cpp :results output
  #include <stdio.h>
  #include <stdlib.h>
  ...
  char* matchesrule(int rulei, char *msg);

  char* matchesrules(char *rulesstring, char *msg) {
    char *ptr = rulesstring - 1;
    while (ptr != NULL) {
      ptr++;
      int r = atoi(ptr);
      msg = matchesrule(r, msg);
      if (msg == NULL) {
        return NULL;
      }
      ptr = index(ptr, ' ');
  ...
  char* matchesrule(int rulei, char *msg) {
    ...
    if (index(rule, '|') != NULL) {
      ...
      rules2 += 2;
        {
          char *out = matchesrules(rules1, msg);
          if (out != NULL) {
    for (int i = 0; i < indent; i++) printf(" ");
      printf("## <1\n");
      indent--;
        ...
        {
          char *out = matchesrules(rules2, msg);
          if (out != NULL) {
    for (int i = 0; i < indent; i++) printf(" ");
      printf("## <1\n");
      indent--;
    ...
    char *out = matchesrules(rule, msg);
    for (int i = 0; i < indent; i++) printf(" ");
    printf("## <%d", out != NULL);
  ...
  int main() {
    ...
    char tmp[10000];
    // 8 : 42 | 42 8
    sprintf(tmp, "42 | 42 %d", ruleslen);
    rules[8] = strdup(tmp);
    for (int i = 0; i < expansions; i++) {
      sprintf(tmp, "42 | 42 %d", ruleslen+1);
      rules[ruleslen++] = strdup(tmp);
    }
    sprintf(tmp, "42 | 42 42", ruleslen+1);
    rules[ruleslen++] = strdup(tmp);

    // 11: 42 31 | 42 11 31
    sprintf(tmp, "42 31 | 42 %d 31", ruleslen);
    rules[11] = strdup(tmp);
    for (int i = 0; i < expansions; i++) {
      sprintf(tmp, "42 31 | 42 %d 31", ruleslen+1);
      rules[ruleslen++] = strdup(tmp);
    }
    sprintf(tmp, "42 31 | 42 42 31 31", ruleslen+1);
    rules[ruleslen++] = strdup(tmp);
...
#+RESULTS:
#+begin_example
## 0(8 11) msg='aaaaabbaabaaaaababaa'
 ## 8(42 | 42 43) msg='aaaaabbaabaaaaababaa'
  ## 42(9 14 | 10 1) msg='aaaaabbaabaaaaababaa'
   ## 9(14 27 | 1 26) msg='aaaaabbaabaaaaababaa'
    ## 14("b") msg='aaaaabbaabaaaaababaa'
    ## <0
    ## 1("a") msg='aaaaabbaabaaaaababaa'
    ## <1
    ## 26(14 22 | 1 20) msg='aaaabbaabaaaaababaa'
     ## 14("b") msg='aaaabbaabaaaaababaa'
     ## <0
     ## 1("a") msg='aaaabbaabaaaaababaa'
     ## <1
     ## 20(14 14 | 1 15) msg='aaabbaabaaaaababaa'
      ## 14("b") msg='aaabbaabaaaaababaa'
      ## <0
      ## 1("a") msg='aaabbaabaaaaababaa'
      ## <1
      ## 15(1 | 14) msg='aabbaabaaaaababaa'
       ## 1("a") msg='aabbaabaaaaababaa'
       ## <1
      ## <1
     ## <1
    ## <1
   ## <1
   ## 14("b") msg='abbaabaaaaababaa'
   ## <0
   ## 10(23 14 | 28 1) msg='aaaaabbaabaaaaababaa'
    ## 23(25 1 | 22 14) msg='aaaaabbaabaaaaababaa'
     ## 25(1 1 | 1 14) msg='aaaaabbaabaaaaababaa'
      ## 1("a") msg='aaaaabbaabaaaaababaa'
      ## <1
      ## 1("a") msg='aaaabbaabaaaaababaa'
      ## <1
     ...
     ## <1
    ## <1
    ## 14("b") msg='aabbaabaaaaababaa'
    ## <0
    ## 28(16 1) msg='aaaaabbaabaaaaababaa'
     ## 16(15 1 | 14 14) msg='aaaaabbaabaaaaababaa'
      ## 15(1 | 14) msg='aaaaabbaabaaaaababaa'
       ## 1("a") msg='aaaaabbaabaaaaababaa'
       ## <1
      ## <1
      ## 1("a") msg='aaaabbaabaaaaababaa'
     ...
     ## 1("a") msg='aaabbaabaaaaababaa'
     ## <1
    ## <1     ## 1("a") msg='aabbaabaaaaababaa'
    ## <1
   ## <1
   ## 1("a") msg='abbaabaaaaababaa'
   ## <1
  ## <1
 ## <1
 ## 11(42 31 | 42 45 31) msg='bbaabaaaaababaa'
  ## 42(9 14 | 10 1) msg='bbaabaaaaababaa'
   ## 9(14 27 | 1 26) msg='bbaabaaaaababaa'
    ## 14("b") msg='bbaabaaaaababaa'
    ## <1
    ## 27(1 6 | 14 18) msg='baabaaaaababaa'
     ## 1("a") msg='baabaaaaababaa'
     ## <0
     ## 14("b") msg='baabaaaaababaa'
     ## <1
     ## 18(15 15) msg='aabaaaaababaa'
      ## 15(1 | 14) msg='aabaaaaababaa'
       ## 1("a") msg='aabaaaaababaa'
       ## <1
      ## <1
      ## 15(1 | 14) msg='abaaaaababaa'
       ## 1("a") msg='abaaaaababaa'
       ## <1
      ## <1
     ## <1     ## <1
   ## <1
   ## 14("b") msg='baaaaababaa'
   ## <1
  ## <1
  ## 31(14 17 | 1 13) msg='aaaaababaa'
   ## 14("b") msg='aaaaababaa'
   ## <0
   ## 1("a") msg='aaaaababaa'
   ## <1
   ## 13(14 3 | 1 12) msg='aaaababaa'
    ## 14("b") msg='aaaababaa'
    ## <0
    ## 1("a") msg='aaaababaa'
    ## <1
    ## 12(24 14 | 19 1) msg='aaababaa'
     ## 24(14 1) msg='aaababaa'
      ## 14("b") msg='aaababaa'
      ## <0
     ## <0      ## 19(14 1 | 14 14) msg='aaababaa'
      ## 14("b") msg='aaababaa'
      ## <0
      ## 14("b") msg='aaababaa'
      ## <0
  ...
  ## 42(9 14 | 10 1) msg='bbaabaaaaababaa'
   ...
   ## <1
  ## <1
  ## 45(42 31 | 42 46 31) msg='aaaaababaa'
   ## 42(9 14 | 10 1) msg='aaaaababaa'
    ## 9(14 27 | 1 26) msg='aaaaababaa'
     ## 14("b") msg='aaaaababaa'
     ## <0
     ## 1("a") msg='aaaaababaa'
     ## <1
     ## 26(14 22 | 1 20) msg='aaaababaa'
      ## 14("b") msg='aaaababaa'
      ## <0
      ## 1("a") msg='aaaababaa'
      ## <1
      ## 20(14 14 | 1 15) msg='aaababaa'
       ## 14("b") msg='aaababaa'
       ## <0
       ## 1("a") msg='aaababaa'
       ## <1
    ...
    ## 14("b") msg='ababaa'
    ## <0
    ## 10(23 14 | 28 1) msg='aaaaababaa'
     ## 23(25 1 | 22 14) msg='aaaaababaa'
      ## 25(1 1 | 1 14) msg='aaaaababaa'
       ## 1("a") msg='aaaaababaa'
       ## <1
       ## 1("a") msg='aaaababaa'
       ## <1
     ...
     ## 14("b") msg='aababaa'
     ## <0
     ## 28(16 1) msg='aaaaababaa'
      ## 16(15 1 | 14 14) msg='aaaaababaa'
       ## 15(1 | 14) msg='aaaaababaa'
        ## 1("a") msg='aaaaababaa'
        ## <1
       ## <1
       ## 1("a") msg='aaaababaa'
    ...
    ## <1
   ## <1
   ## 31(14 17 | 1 13) msg='babaa'
    ## 14("b") msg='babaa'
    ## <1
    ## 17(14 2 | 1 7) msg='abaa'
     ## 14("b") msg='abaa'
     ## <0
     ## 1("a") msg='abaa'
     ## <1
...
#+end_example
 ...
 - 160 too low <2020-12-22 Tue 18:04>
** part 2 B
#+BEGIN_SRC cpp :results output
  #include <stdio.h>
  #include <stdlib.h>
  ...
  void matches(char *rulesstring, char *msg, int *found) {
    indent++;
    if (rulesstring[0] == 0 && msg[0] == 0) {
      ,*found = true;
      indent--;
      return;
    }

    int r = atoi(rulesstring);
    char *next = index(rulesstring, ' ');
    if (next != NULL) next++; else next = "";

    char *rule = rules[r];
    p("## rule %d='%s', msg=%s, rest='%s', found=%d\n", r, rule, msg, next, *found);
    if (rule[0] == '"') {
      if (rule[1] == msg[0])
        matches(next, msg+1, found);
      indent--;
      return;
    }
    if (index(rule, '|') != NULL) {
      ...
      char *tmp2 = (char*)malloc(sizeof(char) * (strlen(rules1) + strlen(rules2) + strlen(next) + 2));
      sprintf(tmp2, "%s %s", rules1, next);
      matches(tmp2, msg, found);
      sprintf(tmp2, "%s %s", rules2, next);
      matches(tmp2, msg, found);
      free(tmp2);
      indent--;
      return;
    ...
    {
      char *tmp2 = (char*)malloc(sizeof(char) * (strlen(rule) + strlen(next) + 2));
      sprintf(tmp2, "%s %s", rule, next);
      matches(tmp2, msg, found);
      free(tmp2);
      indent--;
      return;
...
: = 321
 ...
 - 160 too low <2020-12-22 Tue 18:04>
 - 321 correct <2020-12-22 Tue 21:01>
