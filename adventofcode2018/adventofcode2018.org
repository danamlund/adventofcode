...
Both parts of this puzzle are complete! They provide two gold stars: **
** part 1
 - <2018-12-16 Sun 19:01>
#+BEGIN_SRC python :results output
  import re
  def ints(line):
    return map(int, re.findall("[0-9]+", line))
  ...
  def f(input):
    ...
    threes = 0
    for i in xrange(0, 99999, 4):
      if not input[i].strip():
        break
      before = ints(input[i])
      opcodedata = ints(input[i+1])
      after = ints(input[i+2])
      matches = 0
      opcodenum, a, b, c = opcodedata
      for opcode in opcodes:
        before_copy = list(before)
        opcode(before_copy, a, b, c)
  ...
  print(f(list(open("16.txt"))))
#+END_SRC

#+RESULTS:
: 521

 - 521 <2018-12-16 Sun 19:23>
** part 2
#+BEGIN_SRC python :results output
  import re
  def ints(line):
  ...
  def f(input):
    ...
    for i in xrange(0, 99999, 4):
      ...
      nums = opcodenums.get(opcodenum, [])
      nums.append(matches)
      opcodenums[opcodenum] = nums
    ...
    while newfound >= 1:
      newfound = 0
      for k,v in opcodenums.items():
        unknowns = [ [ i for i in m if i not in founds.values() ] for m in v ]
        unknown_distincts = [ d for d in unknowns if len(d) == 1 ]
        if unknown_distincts:
          founds[k] = unknown_distincts[0][0]
          newfound += 1

    regs = [0, 0, 0, 0]
    for i in xrange(before_afters*4+2, len(input)):
      num, a, b, c = ints(input[i])
      founds[num](regs, a, b, c)
    return regs[0]
...
#+BEGIN_SRC cpp :results output
  ...
  int read(FILE *f) {
    ...
    char line[200];
    while (NULL != fgets(line, sizeof(line), f) && strlen(line) > 3) {
      int xstart, xend;
      int ystart, yend;

      int secondIndex;
      for (secondIndex = 0; line[secondIndex - 1] != ' '; secondIndex++);
      line[secondIndex - 2] = '\0';

      for (int i = 0; i <= secondIndex; i += secondIndex) {
        int start, end;

        start = atoi(line + i + 2);
        char *lineend = strstr(line + i + 2, "..");
        if (NULL != lineend) {
          end = atoi(lineend + 2);
        } else {
          end = start;
      ...
      for (int y = ystart; y <= yend; y++) {
        ...
        for (int x = xstart; x <= xend; x++) {
          if (x-1 < mapx0) mapx0 = x-1;
          if (x+1 > mapx1) mapx1 = x+1;
          map[y][x] = '#';
        }
      }
    }
  ...
  int fill(int x, int y) {
    while (y <= mapy1) {
      ...
      if (blocks(x, y+1)) {
        ...
        int lefti;
        int lefttype = 0;
        for (lefti = 0; x-lefti >= mapx0; lefti++) {
          lefttype = type(x-lefti, y);
          if (lefttype) break;
          map[y][x-lefti] = '|';
        }
        ...
        int righti;
        int righttype = 0;
        for (righti = 0; x+righti <= mapx1; righti++) {
          righttype = type(x+righti, y);
          if (righttype) break;
          map[y][x+righti] = '|';
        }

        int out = 0;
        // printf("#### %d,%d  %d,%d\n", lefti, lefttype, righti, righttype);
        ...
        if (lefttype == 2) out += fill(x - lefti, y);
        if (righttype == 2) out += fill(x + righti, y);
...
: = 31412
 ...
 - 31402 too low <2018-12-22 Sat 17:08>
 - 31418 too high <2018-12-22 Sat 17:13>
 - 31414 too high <2018-12-22 Sat 17:14>
 - 31412 correct <2018-12-22 Sat 17:16>
...
#+RESULTS:
: = 25857
 - 25857 correct <2018-12-22 Sat 17:17>
* day 18
** puzzle
...
#+BEGIN_SRC cpp :results output
  ...
  int adjacents(int x, int y, char match) {
    int seen = 0;
  ...
  void turn() {
    for(int y = 0; y <= height; y++) {
      for (int x = 0; x <= width; x++) {
        int grounds = adjacents(x, y, '.');
        int trees = adjacents(x, y, '|');
        int lumberyards = adjacents(x, y, '#');
        if (map[y][x] == '.' && trees >= 3)
          map2[y][x] = '|';
        else if (map[y][x] == '|' && lumberyards >= 3)
          map2[y][x] = '#';
        else if (map[y][x] == '#' && (lumberyards == 0 || trees == 0))
          map2[y][x] = '.';
        else
          map2[y][x] = map[y][x];
        // printf("%d,%d%c%c: .%d |%d #%d\n", x, y, map[y][x], map2[y][x], grounds, trees, lumberyards);
      }
    }
    swapmaps();
  }
...
#+RESULTS:
: = 480150
 - 480150 correct <2018-12-23 Sun 21:33>
** part 2
#+BEGIN_SRC cpp :results output
  #include <stdio.h>
  ...
  int main(int argc, char **args) {
    ...
    for(; left > matchlen; left -= matchlen);
...
: = 233020
 ...
 - 233020 correct <2018-12-23 Sun 21:55>
...
Your puzzle answer was 28137600.

Both parts of this puzzle are complete! They provide two gold stars: **
** part 1
 - <2018-12-24 Mon 11:08>
#+BEGIN_SRC python :results output
  import re
  def ints(line):
    return map(int, re.findall("[0-9]+", line))
  ...
  def f(input, ip):
    ...
    input = [ line for line in input if line[0] != "#" ]

    regs = [0, 0, 0, 0, 0, 0]

    #ip=11 [0, 11, 0, 930, 11, 1] seti 2 4 4 [0, 11, 0, 930, 2, 1]

    #ip=13 [1, 931, 1, 930, 13, 3] gtrr 5 3 2 [1, 931, 0, 930, 13, 3]

    #while regs[ip] < len(input) and regs[ip] >= 0:
    for i in xrange(6922935):
      line = input[regs[ip]].split()
      opcode = [ code for code in opcodes if line[0] == code.__name__ ][0]
      a,b,c = map(int, line[1:])
      oldregs = list(regs)
      oldip = regs[ip]
      # if regs[ip] == 11 and regs[1] == 5:
      #    regs[1] = regs[3];
      # if regs[ip] == 11 and regs[1] == 5 and regs[5] >= 1:
      #    regs[1] = regs[3]/regs[5]
      # if regs[ip] == 11 and regs[1] == regs[3]/regs[5]+2:
      #    regs[1] = regs[3]
      # if regs[ip] == 13 and regs[5] == 10:
      #   regs[5] = regs[3]
      opcode(regs, a, b, c)
      ii = 17376
      #if ii+1000 > i > ii-100:
      ...
      #if regs[0] == 6:
      #if regs[0] != oldregs[0]:
      #  print "%5d ip=%2d %25s %s %d %d %d %25s" % (i, oldip, oldregs, opcode.__name__, a, b, c, regs)
      regs[ip] += 1
      if regs[ip] >= len(input) or regs[ip] < 0:
        print "%5d ip=%2d %25s %s %d %d %d %25s" % (i, oldip, oldregs, opcode.__name__, a, b, c, regs)
        return regs[0]

    return None

  #print(f(["#ip 0", "seti 5 0 1", "seti 6 0 2", "addi 0 1 0", "addr 1 2 3", "setr 1 0 0", "seti 8 0 4", "seti 9 0 5"], 0))
  #print(f(list(open("19.txt")), 4))
...
: 2304
 ...
 - 2304 correct <2018-12-24 Mon 11:37>
** part 2
 - <2018-12-24 Mon 11:08>
#+BEGIN_SRC python :results output
  import re
  ...
  def f(input, ip):
    ...
    #ip=9 [0, 6, 0, 10551330, 9, 1] gtrr 1 3 2 [0, 6, 0, 10551330, 9, 1]
    #ip=9 [0, 123, 0, 10551330, 9, 1] gtrr 1 3 2 [0, 123, 0, 10551330, 9, 1]
    #regs = [0, 10551330, 0, 10551330, 9, 1]

    #ip=13 [0, 10551331, 1, 10551330, 13, 3] gtrr 5 3 2 [0, 10551331, 0, 10551330, 13, 3]
    #ip=13 [1, 10551331, 1, 10551330, 13, 2] gtrr 5 3 2 [1, 10551331, 0, 10551330, 13, 2]
    #regs = [1, 10551331, 1, 10551330, 13, 10551331]
    ...
    #for _ in xrange(1000):
      ...
      oldregs = list(regs)
      oldip = regs[ip]
      if oldip == 9 and regs[1] == 5:
        regs[1] = 10551331
      if oldip == 13 and regs[5] == 5:
        regs[5] = 10551331
      opcode(regs, a, b, c)
      print "ip=%d %s %s %d %d %d %s" % (oldip, oldregs, opcode.__name__, a, b, c, regs)
      regs[ip] += 1
...
: 28137600
 ...
 - 0 wrong <2018-12-24 Mon 12:09>
 - 28137600 correct <2018-12-24 Mon 13:35>
* day 20
** puzzle
...
Your puzzle answer was 8545.

Both parts of this puzzle are complete! They provide two gold stars: **
** part 1
#+BEGIN_SRC cpp :results output varbatim
  #include <stdio.h>

  #define M 1000
  unsigned char map[M][M];
  int xmin = M/2;
  int xmax = M/2;
  int ymin = M/2;
  int ymax = M/2;

  void seenpos(int x, int y) {
    if (x < xmin) xmin = x;
    if (x > xmax) xmax = x;
    if (y < ymin) ymin = y;
    if (y > ymax) ymax = y;
  }
  void adddoor(int x, int y, char c) {
    unsigned char dir = 0;
    if (c == 'N') { adddoor(x, y-1, 'S'); return; }
    if (c == 'E') { adddoor(x+1, y, 'W'); return; }
    if (c == 'S') { dir += 1; seenpos(x,y); seenpos(x,y+1); }
    if (c == 'W') { dir += 2; seenpos(x,y); seenpos(x-1,y); }
    map[y][x] |= dir;
  }
  int isdoor(int x, int y, char c) {
    if (c == 'N') return isdoor(x, y-1, 'S');
    if (c == 'E') return isdoor(x+1, y, 'W');
    if (c == 'S') return (map[y][x] & 1);
    if (c == 'W') return (map[y][x] & 2);
  }
  ...
  void printmap() {
    ...
    for (int y = ymin; y <= ymax; y++) {
      for (int x = xmin; x <= xmax; x++) {
        printf("#%c", isdoor(x, y, 'N')?'-':'#');
      }
      printf("#\n");
      for (int x = xmin; x <= xmax; x++) {
        if (x == M/2 && y == M/2)
          printf("%cX", isdoor(x,y,'W')?'|':'#');
        else
          printf("%c.", isdoor(x,y,'W')?'|':'#');
      }
      printf("#\n");
    }
    for (int x = xmin; x <= xmax; x++) {
  ...
  int parse(int x, int y, int i, const char *s) {
    while (true) {
      // printf("%3d,%3d %d=%c\n", M/2-x, M/2-y, i, s[i]);
      if (s[i] == '^') i++;
  ...
  void filldists(int x, int y, int dist, int dists[M][M]) {
    if (dists[y][x] >= 0) return;
    dists[y][x] = dist;
    if (isdoor(x, y, 'N')) filldists(x, y-1, dist+1, dists);
    if (isdoor(x, y, 'E')) filldists(x+1, y, dist+1, dists);
    if (isdoor(x, y, 'S')) filldists(x, y+1, dist+1, dists);
    if (isdoor(x, y, 'W')) filldists(x-1, y, dist+1, dists);
  }
  ...
  int main(int argc, char **args) {
    printf("= %d\n", furthest(fopen("20.txt", "r")));
    // printf("= %d\n", furthest(fopen("20_example3.txt", "r")));
    return 0;
  }
#+END_SRC
